...
/*   Updated: 2022/09/13 15:36:53 by jsaarine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	find_normal(t_context *ctx)
{
	...
	if (ctx->obj.type  == SPHERE)
		ctx->hit.normal = get_sphere_normal(ctx->obj.loc, ctx->ray, ctx->hit.closest_distance);
	
	else if (ctx->obj.type == PLANE)
		ctx->hit.normal = get_plane_normal(ctx->obj, ctx->ray);
	else if (ctx->obj.type == CYLINDER)
		ctx->hit.normal = get_cylinder_normal(ctx->obj, ctx->ray, ctx->hit.closest_distance);
	else if (ctx->obj.type == CONE)
		ctx->hit.normal = get_cone_normal(ctx->obj, ctx->ray, ctx->hit.closest_distance);
}
