...
/*   Updated: 2022/09/15 19:30:48 by jsaarine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_abc static	calc_abc_cylinder(t_vec3 raydir, t_vec3 cyldir, t_vec3 raycyl, double radius)
{
	...
	abc.b = 2 * (vec3_dot(raydir, raycyl) - vec3_dot(raydir, cyldir)
...
t_abc static	calc_abc_cylinder_2(t_ray ray, t_vec3 cyldir, t_vec3 raycyl, double radius)
{
	t_abc	abc;
...
t_abc static	calc_abc_cylinder_3(t_ray ray, t_vec3 cyldir, t_vec3 raycyl, double radius)
{
	t_abc	abc;
...
int	intersects_cylinder(t_ray *ray, t_object *cylinder, double *distance)
{
	...
	//abc = calc_abc_cylinder_2(*ray, cylinder->rot, ray_cyl, cylinder->size);
	abc = calc_abc_cylinder_3(*ray, cylinder->rot, ray_cyl, cylinder->size);
